...
export default function AdminPortal() {
  ...
  if (!token) {
    return <main className="auth-shell">
      ...
      <form className="auth-card" onSubmit={login}>
        <header><span className="brand-shield"><ShieldCheck /></span><div><strong>LEAK//SHIELD</strong><small>RESTRICTED CONTROL PLANE</small></div></header>
        <div className="auth-clearance"><LockKeyhole /><span>LEVEL 04 // OPERATOR AUTHENTICATION</span></div>
        <h1>Identify yourself.</h1>
        <p>This route is isolated from public navigation. Authorized operators only.</p>
        <label className="form-field"><span>Operator email</span><input required autoComplete="username" type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label>
        <label className="form-field"><span>Passphrase</span><input required autoComplete="current-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>
        {error && <div className="alert alert-error"><AlertTriangle /><span>{error}</span></div>}
        <button className="primary-button primary-button-large" disabled={loading} type="submit">{loading ? <Loader2 className="spin" /> : <LockKeyhole />} ENTER CONTROL PLANE</button>
        <a className="text-button auth-return" href="/">Return to public console</a>
      </form>
    </main>;
  }
  ...
  return <main className="admin-console">
    <header className="admin-topbar">
      <div><span className="brand-shield"><ShieldCheck /></span><span><strong>LEAK//SHIELD</strong><small>CONTROL PLANE</small></span></div>
    ...
    <div className="admin-canvas">
      <header className="admin-heading"><p>RESTRICTED // REDACTED AUDIT STORAGE</p><h1>Operator Control Plane</h1><span>Review privacy-safe user sessions and the scan results returned to each browser.</span></header>
      {error && <div className="alert alert-error"><AlertTriangle /><span>{error}</span></div>}
      ...
      <section className="admin-layout">
        <aside className="panel admin-users"><header><UserRound /><span><strong>SESSION GROUPS</strong><small>{storage?.provider || "storage unavailable"}</small></span></header><div>{users.map((user) => <button className={selectedUser?.id === user.id ? "active" : ""} key={user.id} onClick={() => setSelectedUserId(user.id)}><span><strong>{user.id}</strong><small>{user.scan_count} scans // {user.finding_count} findings</small></span><b>{user.latest_risk || "LOW"}</b></button>)}{!users.length && <p className="admin-empty">{loading ? "Loading encrypted index..." : "No audit records yet."}</p>}</div></aside>
        <section className="panel admin-record-view">
          <header><Database /><span><strong>{selectedUser?.id || "NO SESSION SELECTED"}</strong><small>{selectedUser ? `${selectedUser.scan_count} stored assessment(s)` : "Choose a session group"}</small></span></header>
